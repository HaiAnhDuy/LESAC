...
class TableUser extends Component {
    ...
    async componentDidMount() {
        ...
        this.setState({
            data_table: this.props.all_data_user.data
        })

    }

    componentDidUpdate(prevProps, prevState) {
        if (prevProps.all_data_user !== this.props.all_data_user) {
            let all_data_user = this.props.all_data_user.data
            this.setState({
                data_table: all_data_user,
    ...
    handleButtoonDelete = async (id) => {
        ...
        await this.props.HandleDeleteDataUserByIdRedux(id);
        await this.props.HandleGetAllDataUser();
                        ...
                        <tbody>
                            {data_table && data_table.length > 0 &&
                                data_table.map((items, index) => {
                                    ...
                                    let gender = ''
                                    let role = ''
                                    if (items && items.GenderData && items.GenderData.valueVi) {
                                        gender = items.GenderData.valueVi
                                    }
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        HandleDeleteDataUserByIdRedux: (id) => dispatch(actions.DeleteDataUserByIdRedux(id)),
